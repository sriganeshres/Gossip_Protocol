 ...
 * seed.cpp
 *
 * This program implements a Seed Node for the Gossip Protocol-based P2P network.
 * The seed node listens for incoming connections from peers.
 * When a peer sends a "Register" message, the seed registers that peer (if not already registered)
 * and sends back its current list of peers (each as IP:Port:degree; separated by semicolons).
 * It also handles "UpdateDegree" and "DeadNode" messages.
 *
 * Compile with:
...
public:
    ...
    // Start the seed server: bind, listen, and spawn threads for each incoming connection.
    void start() {
...
private:
    ...
    void processMessage(const std::string &msg, int clientSock) {
        ...
        if (command == "Register") {
            ...
            // Expected format: DeadNode:<deadIP>:<deadPort>:<timestamp>:<reporterIP>:<reporterPort>
